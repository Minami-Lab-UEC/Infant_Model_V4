...
def plot_history(epochs, acc, i, j, figure_name='onememory_except_nounonly10000'):
    ...
    plt.ylabel('accuracy')
    plt.ylim([-0.02,1.02])
    plt.savefig(DIR_PATH+'figure_'+figure_name+'_' + datetime.now().strftime('%Y%m%d') + f'_{i}_{j}' + '.png')
    ...
    for i in range(2):
        ...
        for episode in range(NUM_EPISODES-10000, NUM_EPISODES):
            ...
            if episode % TEST_EPOCHS_INTERVAL == 0:
                print('test!')
                reward_sum, reward_sum_n, reward_sum_v = mainQN.test_onememory_except(Data, actor, symbols, symbols_noun, symbols_verb, actions,  
                ...
                acc = np.append(acc, reward_sum/(TEST_EPOCHS))
                print(f'test accuracy : {reward_sum/(TEST_EPOCHS)}')
                epochs = np.append(epochs, epoch)
                
                plot_history(epochs, acc, i, j)
            ...
            # if episode % 100 == 0:
                # mainQN.model.save_weights(DIR_PATH+f'check_points/my_checkpoint_onememory_except_nounonly10000_{episode}_{i}')

            # print(name_select_list)
            # print(feature_select_list)
        np.save(DIR_PATH + 'numpy/' + f'proposed_onememory_except_nounonly10000_name_{NUM_EPISODES}_{i}_{j}.npy', name_select_list)
        np.save(DIR_PATH + 'numpy/' + f'proposed_onememory_except_nounonly10000_feature_{NUM_EPISODES}_{i}_{j}.npy', feature_select_list)

        np.save(DIR_PATH + 'numpy/' + f'proposed_onememory_except_nounonly10000_acc_{NUM_EPISODES}_{i}_{j}.npy', acc)
        np.save(DIR_PATH + 'numpy/' + f'proposed_onememory_except_nounonly10000_epochs_{NUM_EPISODES}_{i}_{j}.npy', epochs)

        mainQN.model.save_weights(DIR_PATH+f'check_points/my_checkpoint_onememory_except_nounonly10000_{NUM_EPISODES}_{i}_{j}')
