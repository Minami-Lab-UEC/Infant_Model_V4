...
def plot_history(epochs, acc, i = 0, figure_name='multiple_output'):
    ...
    plt.ylabel('accuracy')
    plt.ylim([-0.02,1.02])
    plt.savefig(DIR_PATH + 'figure_' + figure_name + '_' + datetime.now().strftime('%Y%m%d') + i + '.png')
...
for episode in range(NUM_EPISODES):
    ...
    if episode % TEST_EPOCHS_INTERVAL == 0:
        ...
        if episode == 1000 or episode == 4000 or episode == 7000:
            mainQN.model.save_weights(DIR_PATH+f'check_points/my_checkpoint_multiple_output_{episode}_{i}')
