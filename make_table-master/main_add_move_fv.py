...
main_df = pd.read_csv('../table/main_new.csv')
...
# # idを削除してindexを振り直す
# main_df = main_df.reset_index(drop=True).drop('id', axis=1)

# # row[1]:id
# # row[2:]:move feature value
# for row in move_df.itertuples(name=None):
# 	# -57600からmove feature valueのデータが保存されている
# 	main_df.iloc[row[1]][-57600:] = row[2:]
...
# 保存
main_df.to_csv('densetrack_create_takeshita/main_new.csv', index=False)
