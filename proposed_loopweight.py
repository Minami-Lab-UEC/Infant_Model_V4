...
def plot_history(epochs, acc, i, figure_name='loopweight'):
...
for i in range(1, 2):
    ...
    for episode in range(NUM_EPISODES):
        ...
        memory_none = [np.concatenate([fea_vec[0], fea_val[0], obj[0], cur_loop]), np.concatenate([fea_vec[0], fea_val[0], obj[0], cur_loop]), 
        ...
        for step in range(MAX_NUMBER_OF_STEPS):
            ...
            for loop in range(MAX_NUMBER_OF_LOOPS):
                ...
                obj[loop][obj_name_idx - actions_length] = 1
                if name == 'not_sure':
                    not_sure_count = 1
                ...
                reward, terminal = reward_func_loop_weight(objectIndex[loop], (obj_name_idx - actions_length), not_sure_count, step, MAX_NUMBER_OF_STEPS,
                ...
                state1 = np.concatenate([fea_vec[loop], fea_val[loop], obj[loop], cur_loop])
                next_out = fea_vec[loop]+obj[loop]
        ...
        if episode == 1000 or episode == 5000 or episode == 8000:
            mainQN.model.save_weights(DIR_PATH+f'check_points/my_checkpoint_loopweight_{episode}_{i}')

        # print(name_select_list)
        # print(feature_select_list)
    np.save(DIR_PATH + 'numpy/' + f'loopweight_name_{NUM_EPISODES}_{i}.npy', name_select_list)
    np.save(DIR_PATH + 'numpy/' + f'loopweight_feature_{NUM_EPISODES}_{i}.npy', feature_select_list)

    mainQN.model.save_weights(DIR_PATH+f'check_points/my_checkpoint_loopweight_{NUM_EPISODES}_{i}')
