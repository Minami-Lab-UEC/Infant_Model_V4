...
def plot_history(epochs, acc, i, j, figure_name='er_except_nounonly10000'):
...
for j in range(10):
    ...
    for i in range(2):
        ...
        for episode in range(NUM_EPISODES-10000, NUM_EPISODES):
            ...
            # if episode % 100 == 0:
                # mainQN.model.save_weights(DIR_PATH+f'check_points/my_checkpoint_er_except_nounonly10000_{episode}_{i}')

            # print(name_select_list)
            # print(feature_select_list)
        np.save(DIR_PATH + 'numpy/' + f'proposed_er_except_nounonly10000_name_{NUM_EPISODES}_{i}_{j}.npy', name_select_list)
        np.save(DIR_PATH + 'numpy/' + f'proposed_er_except_nounonly10000_feature_{NUM_EPISODES}_{i}_{j}.npy', feature_select_list)

        np.save(DIR_PATH + 'numpy/' + f'proposed_er_except_nounonly10000_acc_{NUM_EPISODES}_{i}_{j}.npy', acc)
        np.save(DIR_PATH + 'numpy/' + f'proposed_er_except_nounonly10000_epochs_{NUM_EPISODES}_{i}_{j}.npy', epochs)

        mainQN.model.save_weights(DIR_PATH+f'check_points/my_checkpoint_er_except_nounonly10000_{NUM_EPISODES}_{i}_{j}')
